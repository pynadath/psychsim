...
class FlashSpec(widget.WidgetSpec):
    def __init__(self):
        events = []
        attributes = {
        ...
        widget.WidgetSpec.__init__( self, 'Flash', 'Widget', events, attributes )
        

class Flash(widget.Widget, FlashWindow):
    """
    show flash.
    """
...
##            name = aResource.name
##            )
##        FlashWindow(self, style=wx.SUNKEN_BORDER)

##        self, parent, ID=-1, pos=wx.DefaultPosition,
##                 size=wx.DefaultSize, style=0, name='FlashWindow'
##                 
        FlashWindow.__init__(
            self,
            aParent, 
            widget.makeNewId(aResource.id),
            aResource.position, 
...
registry.Registry.getInstance().register(sys.modules[__name__].Flash)
