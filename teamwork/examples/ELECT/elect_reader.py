...
### each agent a name, a superclass, state values (wealth, security, power) and
### goal weights. Goal weights have the form relation,wealth,securtiy,power
### where relation can be self or any defined relation - so an agent can have
### the goal of increasing its own or the power of its community, etc.
...
### relation,wealth,security,power. Note each action must result in multiple
...
# Construct the agent's goals
            ...
            for statekey in ['money', 'security', 'power']:
        ...
        classHierarchy[name] = {
            'parent':['Entity'],
            'state':{'money':float(m.group('money')),
                     'security':float(m.group('security')),
...
# move to the end after all the actions have been processed
    ...
    def buildStaticActions(self,entity):
        if classHierarchy[entity]['dynamics'].has_key('negSatisfaction'):
            classHierarchy[entity]['dynamics']['negSatisfaction']['reject'] = {'class':PWLDynamics,'args':setToConstant('negSatisfaction',-.01)}
        ...
        if m.group('type') == 'offer':
            ...
            for statekey in ['money', 'security', 'power']:
                if dmat.group(statekey) != '':
                    ...
                    if self.aggregateDelivery and delta > 1e-5:
...
### state feature to keep track of what is on the table
            tmpdyn[negotfeat] ={negotfeat:{'class':PWLDynamics,
...
##		{'class':PWLDynamics,
##		 'args':conditionalSet('actor',unpairact,0.0,[makeStateKey('object',otherfeat)],[])}
##	    tmpdyn[unpairact][otherfeat] =\
##		{'class':PWLDynamics,
##		 'args':conditionalSet('object',unpairact,0.0,[makeStateKey('self',negotfeat)],[])}
	    

        elif m.group('type') == 'threat':
            tmpobjnegsat['threat-'+m.group('act')]= {'class':PWLDynamics,
                                                     'args':increment('negSatisfaction',-.1)}
...
# we add the same dynamics to all entities
    ...
    def deliverDynamics(self,agent,feature,action,actor,delta):
        ...
        @type agent: str
        @param feature: the state feature being updated (e.g., money, power, security)
        @type feature: str
        ...
        try:
            ...
            tree = ProbabilityTree(IdentityMatrix(feature))
##            tree.getValue()[key] = UnchangedRow(sourceKey=key)
        ...
        if leaf is None:
            ...
            leaf = ProbabilityTree(IdentityMatrix(feature))
            tree.branch(plane,copy.deepcopy(tree),leaf)
##            new.getValue()[key] = vector
        # Check the flag for this offer
        deltaKey = StateKey({'entity':'actor','feature':'offer-%s' % (action)})
        if isinstance(leaf.getValue(),IdentityMatrix):
            leaf.makeLeaf(KeyedMatrix())
        leaf.getValue().set(key,deltaKey,delta)
