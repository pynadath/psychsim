...
if __name__ == '__main__':
    ...
    hurricane = 0
    phase = 'none'
    states = [{'Nature': {name: world.agents[name].getBelief() for name in actors}}]
    ...
    for hurricane in hurricanes:
        ...
        pool = {name for name in actors if world.getState(name,'alive',states[hurricane['Landfall']-1]['Nature']['__state__']).first()}
        ...
        for name in sample:
            ...
            entry.update(accessibility.getCurrentDemographics(name,world,states,config,t))
            entry['At Shelter'] = 'yes' if accessibility.getInitialState(name,'location',world,states,t).first() in shelterObjs else 'no'
            entry['Evacuated'] = 'yes' if accessibility.getInitialState(name,'location',world,states,t).first() == 'evacuated' else 'no'
            entry['Category'] = int(round(accessibility.getInitialState('Nature','category',world,states,t,name).expectation()))
            entries.append(entry)
            belief = copy.deepcopy(next(iter(states[t-1]['Nature'][name].values())))
            pEvac = []
            pShelter = []
            risks = []
            while world.getState('Nature','phase',belief).first() != 'none':
                if name in world.next(belief):
                    ...
                    decision = agent.decide(belief,selection='distribution')
                    pShelter.append(0.)
                    for action,prob in decision['action'].items():
                        if action['verb'] == 'evacuate':
                            pEvac.append(prob)
                        elif action['verb'] == 'moveTo' and action['object'] in shelterObjs:
                            pShelter[-1] = max(prob,pShelter[-1])
                world.step(state=belief,select=True,keySubset=belief.keys())
    ...
    for hurricane in hurricanes:
        ...
        if hurricane['Hurricane'] < len(hurricanes):
            ...
            pool = {name for name in actors 
                if world.getState(name,'alive',states[hurricanes[hurricane['Hurricane']]['Start']]['Nature']['__state__']).first()}
            end = hurricanes[hurricane['Hurricane']]['Start']
        else:
            # Pool people who are alive at the end of the simulation
            pool = {name for name in actors if world.getState(name,'alive',states[-2]['Nature']['__state__']).first()}
            end = len(states)-1
        ...
        for name in sample:
            ...
            entry = {'Name': name,'Timestep': t,'Hurricane': hurricane['Hurricane']}
            entry.update(accessibility.getCurrentDemographics(name,world,states,config,t))
            actions = accessibility.getAction(name,world,states,(hurricane['Start'],hurricane['End']+1))
            locations = {dist.first() for dist in accessibility.getInitialState(name,'location',world,states,(hurricane['Start'],hurricane['End']+1))}
            entry['At Shelter'] = 'yes' if locations & shelterObjs else 'no'
            entry['Evacuated'] = 'yes' if 'evacuated' in locations else 'no'
            health = [dist.first() for dist in accessibility.getInitialState(name,'health',world,states,(hurricane['Start'],hurricane['End']+1))]
            entry['Injured'] = 'yes' if min(health) < 0.2 else 'no'
            risk = [dist.expectation() for dist in accessibility.getInitialState(name,'risk',world,states,(hurricane['Start'],hurricane['End']+1),name)]
            entry['Risk'] = accessibility.toLikert(max(risk),7)
            entry['Dissatisfaction'] = accessibility.toLikert(accessibility.getInitialState(name,'grievance',world,states,t).first(),7)
            pShelter = []
            pEvac = []
            pHome = []
            for day in range(hurricane['Start'],hurricane['End']+1):
                belief = copy.deepcopy(next(iter(states[day-1]['Nature'][name].values())))
                decision = agent.decide(belief,selection='distribution')
                pEvac.append(0.)
                pShelter.append(0.)
                pHome.append(0.)
