...
class Group(Agent):
    def __init__(self,name,world,config):
        ...
        if config.getboolean('Groups','prorisk'):
            ...
            #//GT: node 46; 1 of 1; next 6 lines
            tree = {'if': equalRow(stateKey('Nature','phase'),'none'),True: False,False: True}
            if name in regions:
                actGoodRisk = self.addAction({'verb': 'decreaseRisk','object': name},makeTree(tree).desymbolize(world.symbols),codePtr=True)
            else:
...
#            goHome = self.addAction({'verb': 'returnHome'},codePtr=True)
        ...
        #//GT: node 47; 1 of 1; next 1 lines
        ...
        #//GT: node 48; 1 of 1; next 1 lines
        ...
        logNode('Group\'s beliefAggregation','Degree to which group\'s aggregate beliefs are biased toward those of its more optimistic/pessimistic members',
            'String: "mean" / "max" / "min"','Static')
        #//GT: node 49; 1 of 1; next 16 lines
        aggregators = ['mean']
        weights = [5]
...
#        size = stateKey(self.name,'size')
        ...
        for name in agents:
            ...
            #//GT: node 50; 1 of 1; next 10 lines
            ...
            #//GT: node 51; 1 of 1; next 5 lines
            ...
            #//GT: edge 63; from 51; to 50; 1 of 1; next 2 lines
...
#            self.world.setDynamics(size,join,tree,codePtr=True)
            ...
            #//GT: node 52; 1 of 1; next 5 lines
            ...
            #//GT: edge 64; from 52; to 50; 1 of 1; next 2 lines
...
#            self.world.setDynamics(size,leave,tree,codePtr=True)
            if self.config.getboolean('Actors','attachment'):
                ...
                #
                if agent.attachment == 'anxious' or agent.attachment == 'avoidant':
                    R = rewardKey(name)
                    tree = makeTree({'if': thresholdRow(stateKey(name,'risk'),
                                                    likert[5][self.config.getint('Actors','attachment_threshold')]),
                                 True: setToFeatureMatrix(R,member,1. if agent.attachment == 'anxious' else -1.),
                                 False: setToConstantMatrix(R,0.)})
                    agent.setReward(tree,likert[5][self.config.getint('Actors','attachment_r')-1])

            logNode('Group\'s magnification','Magnification of effect of joint action by Group (as a function of size of the group)','Real in [0-1]','Static')
            #//GT: node 53; 1 of 1; next 1 lines
                        ...
                        for lonely in agent.actions:
                            if lonely['verb'] == action['verb'] and \
                               (action.get('object') is None or \
                                ...
                                logEdge('Group-decreaseRisk','Region\'s risk','sometimes','Performing prosocial behavior reduces the risk in the target region')
                                #//GT: edge 65; from 46; to 1; 1 of 1; next 15 lines
                                ...
                                #//GT: edge 66; from 50; to 1; 1 of 1; next 13 lines
                                ...
                                #//GT: edge 67; from 53; to 1; 1 of 1; next 11 lines
...
#        self.setState('size',count)
    ...
    def members(self,state=None):
        ...
        return [agent for agent in self.potentials \
                if binaryKey(agent,self.name,'memberOf') in state and self.world.getFeature(binaryKey(agent,self.name,'memberOf'),state,True)]

    def reward(self,state=None,model=None,recurse=False):
        logNode('Group\'s Expected Reward','Expected utility (desirability) of candidate actions','Real for each action choice')
        #//GT: node 56; 1 of 1; next 26 lines
        logEdge('Actor\'s Expected Reward','Group\'s Expected Reward','often','Expected rewards of joint actions derived from individual members\' expected rewards')
        #//GT: edge 73; from 35; to 56; 1 of 1; next 24 lines
        logEdge('Actor memberOf Group','Group\'s Expected Reward','often','Expected rewards of joint actions derived from individual members\' expected rewards')
        #//GT: edge 74; from 50; to 56; 1 of 1; next 22 lines
        if state is None:
            state = self.world.state
        if model is None:
        ...
        for name in self.members(state):
            ...
            R = agent.getReward()
            assert len(R) == 1
            tree = next(iter(R.values()))
            if tree.isLeaf():
                vector = tree.getLeaf()[makeFuture(rewardKey(name))]
            else:
                # Should be only because of attachment adding a reward for group membership, which is a moot point now
                vectors = [subtree[makeFuture(rewardKey(name))] for subtree in tree.leaves()]
                vector = min(vectors,key=lambda l: len(l))
            ER = 0.
            for key,weight in vector.items():
                if key != CONSTANT:
                    ER += weight*self.world.getFeature(key,state).expectation()
            total += ER
        return total
    ...
    def decide(self,state=None,horizon=None,others=None,model=None,selection=None,actions=None,
        ...
        for action in actions:
            logEdge('GroupBeliefOfRegion\'s risk','Group-%s' % (action['verb']),'often','Group decisions are influenced by their beliefs of the risk')
            #//GT: edge 92; from 55; to 46; 1 of 1; next 33 lines
            logEdge('Group\'s horizon','Group-%s' % (action['verb']),'often','Group decisions are influenced by how far into the future they consider')
            #//GT: edge 93; from 48; to 46; 1 of 1; next 31 lines
        ...
        for action in actions:
            ...
            for t in range(horizon):
                self.world.step({self.name: action},current,keySubset=belief.keys(),select='max',updateBeliefs=False)
                ...
                self.world.agents['Nature'].step('max',current,False)
