...
if __name__ == '__main__':
    logging.basicConfig(level=logging.INFO,filename=os.path.join(os.path.dirname(__file__),'TA2B-TA1C-54.log'))
    ...
    for instance in range(2,3):
        ...
        world.agents['System'].TA2BTA1C54 = True
        world.agents['System'].TA2BTA1C52 = False
        output = []
        # Who would gov't have given aid to in the past under Condition 1?
        t0 = 1 if args['span'] < 365 else 365
        targets = {}
        for t in range(t0,args['span']+1):
            regions = sorted([(len([name for name in pool if data[name][stateKey(name,'alive')].get(t,False) and \
                data[name][stateKey(name,'location')][t] != demos[name]['Residence']]),region) for region,pool in residents.items()])
            targets[t] = {entry[1] for entry in regions if entry[0] == regions[-1][0]}
        for name in pool:
            ...
            oldGrievance = data[name][griev][t0]
            current = oldGrievance
            tree = next(iter(world.dynamics[griev].values()))
            weight = tree.children[True].getLeaf()[makeFuture(griev)][griev]
            inc = tree.children[False].getLeaf()[makeFuture(griev)][CONSTANT]
            for t in range(t0,max(targets.keys())):
                if demos[name]['Residence'] in targets[t]:
                    current *= weight
                else:
                    current = weight*current+inc
            record['Condition 1 Satisfaction'] = responses[accessibility.toLikert(current)]
            # 2-6
        ...
        accessibility.writeOutput(args,output,fields,'TA2B-TA1C-54AidAllocationShelterEvac.tsv',
