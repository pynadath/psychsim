...
if __name__ == '__main__':
    logging.basicConfig(level=logging.INFO,filename=os.path.join(os.path.dirname(__file__),'TA2B-TA1C-56.log'))
    participants = {instance+1: {} for instance in range(14)}
    instance = None
    with open(os.path.join(os.path.dirname('__file__'),'..','TA2B-TA1C-54','TA2B-TA1C-54.log'),'r') as infile:
        for line in infile:
            if 'Instance' in line:
    ...
    for instance in range(1,15):
        ...
        accessibility.writeOutput(args,output,fields,'TA2B-TA1C-56statusquofor54.tsv',
