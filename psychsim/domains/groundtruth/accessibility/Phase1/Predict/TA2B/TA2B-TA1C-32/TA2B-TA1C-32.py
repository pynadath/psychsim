...
def pairedPreSurvey(entry,data,hurricanes,participants,sub='Input'):
    fields = None
    outputOld = []
    outputNew = []
    responses = {name: set() for name in participants.values()}
    with accessibility.openFile(entry,os.path.join(sub,'ActorPreTable.tsv')) as csvfile:
    ...
    for i in range(len(outputOld)):
        ...
        if hurricane['Hurricane'] in responses[name]:
            ...
            for j in range(i+1,len(outputOld)):
                ...
                if participants[int(oldRow['Participant'])] == name:
    ...
    return sorted(outputOld+outputNew,key=lambda row: (int(row['Timestep']),int(row['Participant']))),fields

def pairedPostSurvey(entry,data,hurricanes,participants,sub='Input'):
    network = accessibility.readNetwork(entry['instance'],entry['run'],sub)
    fields = None
    outputOld = []
    outputNew = []
    responses = {name: set() for name in participants.values()}
    with accessibility.openFile(entry,os.path.join(sub,'ActorPostTable.tsv')) as csvfile:
        reader = csv.DictReader(csvfile,delimiter='\t')
        for row in reader:
            name = participants[int(row['Participant'])]
            hurricane = hurricanes[int(row['Hurricane'])-1]
...
if __name__ == '__main__':
    ...
    for instance in range(2,len(accessibility.instances)):
        ...
        output,fields = pairedPreSurvey(entry,data,hurricanes,idMap['ActorPreTable'])
        # Save Data
        ...
        output,fields = pairedPostSurvey(entry,data,hurricanes,idMap['ActorPostTable'])
        # Save Data
