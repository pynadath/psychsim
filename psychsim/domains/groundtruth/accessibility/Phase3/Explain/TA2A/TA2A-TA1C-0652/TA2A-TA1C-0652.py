...
if __name__ == '__main__':
    ...
    for instance,args in accessibility.instanceArgs('Phase3'):
        ...
        for partID in range(len(sample)):
            ...
            var = '%s %d Aid Willingness Employed' % (team,reqNum)
            if var not in variables:
                variables[var] = {'Name': var,'Values':'[0-7]','DataType': 'Integer','Notes': 'Q5'}
            record = dict(root)
            record['VariableName'] = var
            record['Value'] = aidWillingness(world,name,lambda a: a.getState('employed',unique=True))
            output.append(record)

            var = '%s %d Aid Willingness Unemployed' % (team,reqNum)
            if var not in variables:
                variables[var] = {'Name': var,'Values':'[0-7]','DataType': 'Integer','Notes': 'Q6'}
            record = dict(root)
            record['VariableName'] = var
            record['Value'] = aidWillingness(world,name,lambda a: not a.getState('employed',unique=True))
            output.append(record)

            var = '%s %d Aid Willingness Pets' % (team,reqNum)
            if var not in variables:
                variables[var] = {'Name': var,'Values':'[0-7]','DataType': 'Integer','Notes': 'Q7'}
            record = dict(root)
            record['VariableName'] = var
            record['Value'] = aidWillingness(world,name,lambda a: a.demographics['pet'])
            output.append(record)

            var = '%s %d Aid Willingness No Pets' % (team,reqNum)
            if var not in variables:
                variables[var] = {'Name': var,'Values':'[0-7]','DataType': 'Integer','Notes': 'Q8'}
            record = dict(root)
            record['VariableName'] = var
            record['Value'] = aidWillingness(world,name,lambda a: not a.demographics['pet'])
            output.append(record)

            var = '%s %d Aid Willingness Wealth 0' % (team,reqNum)
            if var not in variables:
                variables[var] = {'Name': var,'Values':'[0-7]','DataType': 'Integer','Notes': 'Q9'}
            record = dict(root)
            record['VariableName'] = var
            record['Value'] = aidWillingness(world,name,lambda a: a.getState('resources',unique=True) < 1/8)
            output.append(record)
            ...
            record = dict(root)
            record['VariableName'] = var
            record['Value'] = aidWillingness(world,name,lambda a: 1/8 <= a.getState('resources',unique=True) < 1/4)
            output.append(record)
