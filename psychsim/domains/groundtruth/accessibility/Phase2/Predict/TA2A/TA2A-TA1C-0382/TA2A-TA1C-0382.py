...
def processHistory(world,name,root,history,output,t,variables,prefix,participants=None):
    ...
    if var not in variables:
        variables[var] = {'Name': var,'Values':'yes,no','DataType': 'Boolean','Notes': 'QII.1.a'}
    for step in range(1,len(history),3):
        root['Timestep'] = t+step//3+1
        action = world.getFeature(actionKey(name),history[step]['state']).first()
...
if __name__ == '__main__':
    ...
    for instance,args in accessibility.instanceArgs('Phase2'):
        ...
        for partID in range(len(sample)):
            ...
            output['RunData'] += processHistory(world,name,record,history[:-1],output,day1-1,variables,'%s %d' % (team,reqNum))
        if not cmd['debug']:
