...
if __name__ == '__main__':
    ...
    for instance,args in accessibility.instanceArgs('Phase2','Predict'):
        ...
        for old in hurricanes[:-1]:
            pool = participants['Pre-survey %d' % (old['Hurricane'])]
            for partID,name in sorted(pool.items()):
                ...
                if accessibility.getInitialState(args,name,'alive',world,states,t).first():
                    logging.info('Participant %d (%s) completing for Predict-survey %d' % (partID,name,hurricane['Hurricane']))
                    output += survey.preSurvey(args,name,world,states,config,t,hurricane,variables,'%d Hurricane %d' % (partID,old['Hurricane']))
                else:
