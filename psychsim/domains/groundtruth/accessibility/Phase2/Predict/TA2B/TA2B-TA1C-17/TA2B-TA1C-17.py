...
def runHurricane(args,world,oldStates,config,sample,hurricanes,day1,variables,output,team,reqNum,condition,explain):
    ...
    if not cmd['debug']:
        for partID in range(len(sample)):
            ...
            root = {'Timestep': day1,'EntityIdx': '%s %d Participant' % (team,reqNum,partID+1)}
            if explain:
                exit()
            else:
                # Presurvey
...
if __name__ == '__main__':
    ...
    for instance,args in accessibility.instanceArgs('Phase2','Predict'):
        ...
        for name in sample:
            agent = world.agents[name]
            wealth = 0.8*world.getState(name,'resources').expectation()
            world.setState(name,'resources',wealth)
            beliefs = agent.getBelief()
            model,belief = next(iter(beliefs.items()))
            world.setState(name,'resources',wealth,belief)
        # Trigger hurricane
