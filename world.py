...
class World:
    ...
    def __init__(self,xml=None):
        ...
        self.state = MultiVectorDistribution()
        self.variables = {}
        self.locals = {}
        self.symbols = {}
        self.symbolList = []
    ...
    def newStep(self,actions=None,state=None,real=True,select=True):
        """
        ...
        @type real: bool
        @param select: if C{True}, then select one outcome (according to likelihood) after projecting all possible outcomes (default is C{True})
        @type select: bool
        """
        projection = timeline.Timeline(self,state,actions)
        projection.expand()
        return projection
    ...
    def step(self,actions=None,state=None,real=True,select=True,keys=None):
        ...
        if isinstance(state,MultiVectorDistribution):
...
#            self.modelGC(False)
    ...
    def newStepFromState(self,state,actions=None,horizon=None,tiebreak=None,updateBeliefs=True,keys=None):
        """
        Compute the resulting states when starting in a given possible world (as opposed to a distribution over possible worlds)
        @type state: L{MultiVectorDistribution}
        """
        outcome = {'old': state,
                   'decisions': {}}
        # Check whether we are already in a terminal state
        termination,live,dead = self.terminated(state)
        if termination + psychsim.pwl.KeyedVector.epsilon > 1.:
            outcome['new'] = outcome['old']
            return outcome
        # Determine the actions taken by the agents in this world
        if actions is None:
        ...
        if not isinstance(outcome['actions'],ActionSet) and not isinstance(outcome['actions'],list):
            ...
            turn = self.next(live)
            if len(turn) > 1:
                raise ValueError('Sorry, I am currently unable to process ambiguity about turns.')
            turn = turn.values()[0]
            for name in outcome['actions'].keys():
                if not (name in turn['agents']):
                    raise NameError('Agent %s must wait for its turn' % (name))
            for name in turn['agents']:
                if not outcome['actions'].has_key(name):
                    # What would <name> do?
                    belief = self.agents[name].getBelief(live)
                    decision = self.agents[name].decide(belief,None,horizon,outcome['actions'],None,tiebreak)
                    outcome['decisions'][name] = decision
                    outcome['actions'][name] = decision['action']
                elif isinstance(outcome['actions'][name],Action):
                    outcome['actions'][name] = ActionSet([outcome['actions'][name]])
    ...
    def newEffect(self,actions,state=None):
        if state is None:
            state = self.state
        for key in state.keyMap.keys():
            print(key)
        raise UserWarning
    ...
    def deltaState(self,actions,old,effects,keys=None):
        ...
        for keySet in self.dependency.getEvaluation():
            ...
            if len(keySet) == 0:
                continue
            if isinstance(old,MultiVectorDistribution):
    ...
    def terminated(self,state=None):
        ...
        if state is None:
            state = self.state
        if isinstance(state,MultiVectorDistribution):
            good,bad = state.separate(self.termination,False)
            prob = 1.
            for substate,dist in state.distributions.items():
                if len(dist) == 0:
                    # No possible vectors for this substate
                    return 1.,good,bad
                else:
                    prob = prob*sum(dist.values())
            return 1.-prob,good,bad
        elif isinstance(state,psychsim.pwl.VectorDistribution):
            ...
            assert isinstance(state,psychsim.pwl.KeyedVector)
...
#                    agent.deleteModel(model)
    ...
    def printState(self,distribution=None,buf=None,prefix='',beliefs=True):
        ...
        if isinstance(distribution,MultiVectorDistribution):
                    ...
                    while subnode:
                        if subnode.nodeType == subnode.ELEMENT_NODE:
                            if subnode.tagName == 'worlds':
                                self.state = MultiVectorDistribution(subnode)
